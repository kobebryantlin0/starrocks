...
TEST_F(HdfsScannerTest, TestOrcGetNextWithDiffEncodeDictFilter) {
    SlotDesc two_diff_encode_orc_desc[] = {{"x", TypeDescriptor::from_primtive_type(PrimitiveType::TYPE_BIGINT)},
    ...
    // since we use dict filter eval cache, we can do filter on orc cvb
    // so actually read rows is 101.
    EXPECT_EQ(scanner->raw_rows_read(), 101);
    scanner->close(_runtime_state);
}
